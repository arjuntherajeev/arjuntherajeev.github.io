...
In this tutorial, we will go over how to effectively use **Pandas** with a real-world example from the Cyber Security domain.
...
df = pd.read\_csv(url, skiprows=8, encoding="latin-1")
...
import datetime
o\_dt = datetime.datetime.strptime(s\_dt,'%Y-%m-%d %H:%M:%S')
type(o\_dt)
# datetime.datetime
...
df.groupby('Threat').size().sort\_values(ascending=False)
'''
Threat
...
df\['Firstseen\_year'\] = df\['Firstseen'\].apply(lambda x: datetime.datetime.strptime(x,'%d-%m-%Y').strftime("%Y"))
...
df\['Firstseen\_year'\] = df\['Firstseen\_year'\].astype('datetime64\[ns\]')
df.dtypes
'''
Firstseen                 object
Threat                    object
...
ax = df\[\['Firstseen\_year','Malware'\]\].groupby('Firstseen\_year').count().sort\_values(by='Firstseen\_year', ascending=False).plot(kind='area', figsize=(20,5))
ax.set\_xlabel("Firstseen Year")
...
df.groupby(\['Firstseen\_year','Threat'\]).size()
'''
Firstseen\_year  Threat           
2015            C2                      37
2016            C2                     709
...
ax = df.groupby(\['Firstseen\_year','Threat'\]).size().unstack().plot(kind='area',stacked=True,figsize=(20,5))
ax.set\_xlabel("Firstseen Year")
ax.set\_ylabel("Number of Malware")
